...
## 배포 선행 조건 (사람 게이트) — 2건 모두 필수

1. **R2 버킷 lifecycle `auto-delete-7d` → 30일 상향.** `/upload` 가 공개 엔드포인트라 배포 즉시 누구나 `expiresInDays=30` 을 보낼 수 있으므로 lifecycle 상향과 배포는 같은 창에서 처리해야 한다.
2. **개인정보 문서 갱신.** `docs/PRIVACY.md:30,36,56` 이 아직 "7일 후 영구 삭제"를 사실로 단언한다 — 30일 옵션이 열리면 공개된 개인정보처리방침이 거짓이 된다. `scripts/check-goal-3.mjs:5` 가 PRIVACY 에 `'7일'` 이 있는지 하드 assert 하므로 **한쪽만 고치면 게이트가 깨진다.** 스토어 카피(`scripts/generate-store-screenshots.mjs:674,676,682`)까지 한 묶음으로 처리할 것(P6-T6.2).
