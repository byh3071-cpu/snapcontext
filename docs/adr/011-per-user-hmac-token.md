...
3. **발급 게이트 3중** (`worker/src/index.ts` `/token` 핸들러):
   - **Origin = `chrome-extension://` 필수** — 아니면 403 (`index.ts:100-102`). **단 Origin 은 완전한 인증이 아니다** — 브라우저 내 JS 에서는 forbidden header 라 위조가 어렵지만 curl 등 비브라우저 요청은 Origin 헤더를 임의로 붙일 수 있다. 이 검증의 목적은 "브라우저 확장 밖의 우발적 호출·CSRF 성 요청을 걸러내는 1차 필터"이지 발급 자체를 봉인하는 게 아니다. 실질적 남용 제어는 **rate-limit(아래)**가 담당하고, 발급된 토큰은 owner 네임스페이스만 만들 뿐 타인 데이터 접근 권한이 없다(ADR-012). `/upload` 에는 이 검증이 없다(공개 익명 업로드 계약 유지).
